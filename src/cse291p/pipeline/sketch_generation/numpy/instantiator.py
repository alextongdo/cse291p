"""Numpy-based sketch generation.

Reimplements:
- mockdown/src/mockdown/instantiation/numpy/instantiator.py (lines 1-163)
"""
...
from cse291p.pipeline.view import IView, Attribute
...
class NumpyConstraintInstantiator(IConstraintInstantiator[NT]):
    ...
    def instantiate(self) -> Sequence[IConstraint]:
        ...
        def gen_templates():
            for pair in self.anchor_mat[aspect_ratio_mat.astype(np.bool_)].stack(level=[0, 1]):
            ...
            for pair in self.anchor_mat[parent_relative_size_mat.astype(np.bool_)].stack(level=[0, 1]):
                yield ConstraintFactory.create(
            ...
            for pair in self.anchor_mat.iloc[:, 0][absolute_size_vec.astype(np.bool_)]:
                yield ConstraintFactory.create(
            ...
            for pair in self.anchor_mat[(offset_pos_mat | align_pos_mat).astype(np.bool_)].stack(level=[0, 1]):
                yield ConstraintFactory.create(
