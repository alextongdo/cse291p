"""Simple and heuristic learning implementations.

Reimplements:
- mockdown/src/mockdown/learning/simple.py (entire file)
"""
...
from cse291p.pipeline.constraint import ConstraintKind, IConstraint
from cse291p.pipeline.constraint.constraint import ConstantConstraint, LinearConstraint
from cse291p.pipeline.bayes.errors import ConstraintFalsified
from cse291p.pipeline.bayes.types import IConstraintLearning, ConstraintCandidate
from cse291p.pipeline.bayes.util import widen_bound
...
class SimpleLearning(IConstraintLearning):
    """Emulates the old learning method, rationalizing outputs."""
    ...
    def learn(self) -> List[List[ConstraintCandidate]]:
        ...
        constants: Dict[IConstraint, Dict[str, sympy.Number]] = {}
        ...
        for template in self._templates:
            ...
            constraint: Optional[IConstraint] = None  # type: ignore
            if isinstance(template, LinearConstraint):
                a_frac = sympy.Rational(values['a']).limit_denominator(self._max_denominator)
                b_frac = sympy.Rational(values['b']).limit_denominator(self._max_denominator)
    ...
    def combine_bounds(self, constraints: List[IConstraint]) -> List[IConstraint]:
        ...
        for c in constraints:
            ...
            if other != c and abs(other.b - c.b) < 5:
                output.add(replace(c, op=operator.eq, b=(other.b + c.b) / 2))
            else:
                output.add(c)
        return list(output)
...
"""Simple Bayesian learning implementation."""

# Placeholder for simple learning
