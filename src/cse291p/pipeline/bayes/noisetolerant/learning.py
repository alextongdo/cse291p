"""Noise-tolerant Bayesian learning.

Reimplements:
- mockdown/src/mockdown/learning/noisetolerant/learning.py (entire file)
"""
...
class NoiseTolerantLearning(IConstraintLearning):
    ...
    def _template_data(self, template: IConstraint) -> pd.DataFrame:
        ...
        if template.kind.is_constant_form:
            ...
            columns = [template.y_id, template.x_id]  # x_id is not None for non-constant forms

        rows = []
        for sample in self.samples:
            rows.append([sample.find_anchor(col).value for col in columns])

        return pd.DataFrame(rows, columns=list(map(str, columns)), dtype=float)
...
class NoiseTolerantTemplateModel(abc.ABC):
    def __init__(self, template: IConstraint, data: pd.DataFrame, config: NoiseTolerantLearningConfig):
        ...
        # If we only have one example, append the sensible default preserving simple behavior.
        ...
        # Handle PerfectSeparationError by tiny noise smudging
        while True:
            try:
    ...
    def learn(self) -> List[ConstraintCandidate]:
        ...
        return list(candidates.apply(lambda row: ConstraintCandidate(
            self.template.subst(a=sympy.Rational(row['a']), b=sympy.Rational(row['b'])),
            row['score']), axis=1))
...
# Placeholder for noise-tolerant learning
