...
def load_scraped_examples(file_path: str | Path) -> list[View]:
    ...
    if "examples" not in data:
        raise ValueError(
            f"Invalid scraped format: missing 'examples' key in {file_path}"
        )
...
def _dict_to_view(d: dict[str, Any]) -> View:
    ...
    if "children" in view_dict and view_dict["children"]:
        view_dict["children"] = [
            _dict_to_view(child) for child in view_dict["children"]
        ]
...
def load_examples_from_json(file_path: str | Path) -> list[View]:
    """
    Load examples from a JSON file (supports both
    scraped format and direct examples list).
