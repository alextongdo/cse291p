...
class TemplateInstantiator:
    ...
    def instantiate(self) -> list[LinearConstraint]:
        """
        Returns constraint templates with unknown parameters:
        - Aspect ratio: anchor.width = a * anchor.height
        - Parent-relative: anchor.width = a * parent.width
        - Constant sizes: anchor.width = b
        - Offset positions: anchor.left = anchor.right + b
        """
        sketches = []

        # Assumes all examples have the same hierarchy structure (isomorphic)
        anchors: list[Anchor] = self.examples[0]._anchors_in_subtree
        n = len(anchors)
        ...
        one_horizontal_one_vertical_matrix = (
            is_horizontal[:, None] & is_vertical[None, :]
        )
        dual_type_matrix = (
            (types[:, None] == "right") & (types[None, :] == "left")
        ...
        for example in self.examples:
            ...
            example_visible_matrix = TemplateInstantiator.compute_visibility_matrix(
                anchors, example
            )
            ...
            for child in example._flattened_views_in_subtree:
                if child != example and len(child.children) > 0:
                    child_visible_matrix = (
                        TemplateInstantiator.compute_visibility_matrix(anchors, child)
                    )
...
class ConditionalTemplateInstantiator:
    def __init__(self, examples: list[View]):
        self.examples = examples
        assert len(self.examples) > 0, "At least one example is required."
